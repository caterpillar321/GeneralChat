    ...
    *,
    ...
    if has_web:
        ...
        parts += [
            ...
            "",
            "**검색 query 작성 시 주의** (실패 자주 일어남):",
            f"- 오늘은 {today.year}년이지만, 학습 시점 연도({today.year - 1}, {today.year - 2})를 query에 박지 마세요.",
            "- 옛 연도가 query에 들어가면 검색엔진이 그 시기 자료를 우선 ranking 하여 최신 정보가 묻힙니다.",
            '- 대신 "최신", "현재", "올해", "지금" 같은 시점 키워드를 쓰거나, ',
            f"  꼭 연도가 필요하면 올해({today.year})를 명시하세요.",
            "- 검색 결과의 게시일을 확인하고, 1년 이상 전이면 추가 query 로 최신 정보 재확인.",
            "",
        ]
    if has_documents:
